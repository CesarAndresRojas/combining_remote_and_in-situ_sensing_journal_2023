...
model_list = [hlp.create_polynomial_regression_models(water_column_f, degrees, [False, False, False, False]) for water_column_f, degrees in zip(water_column_f_list, degrees_list)]
...
for i in range(rowIDs.max() + 1):
    for j in range(colIDs.max() + 1):
        ...
        model_lat, model_lon, SDoD_poly, SDoD_regr, SCD_poly, SCD_regr, STempD_poly, STempD_regr, STurbD_poly, STurbD_regr = hlp.get_closest_model(lat, lon, model_list)

        SDoD_regr.intercept_ = surface_do
        SCD_regr.intercept_ = surface_chla
        STempD_regr.intercept_ = surface_temp
        STurbD_regr.intercept_ = surface_turb
        for k in range(global_max_depth):
            if max_depth > 0 and k < max_depth: 
                input_list = [[k]]
                # input_scaled = scaler.transform(input_list)

                input_poly = SDoD_poly.transform([[depth_scale[k][0]]])
                predicted_do = SDoD_regr.predict(input_poly)[0]
                predicted_do = np.maximum(predicted_do, constants.extreme_min_do)

                input_poly = SCD_poly.transform([[depth_scale[k][0]]])
                predicted_chla = SCD_regr.predict(input_poly)[0]
                predicted_chla = np.maximum(predicted_chla, constants.extreme_min_chla)

                if water_mask2 == 1:
                    input_poly = STempD_poly.transform([[depth_scale[k][0]]])
                    predicted_temp = STempD_regr.predict(input_poly)[0]
                    predicted_temp = np.maximum(predicted_temp, constants.extreme_min_temp)
                else:
                    predicted_temp = 0

                input_poly = STurbD_poly.transform([[depth_scale[k][0]]])
                predicted_turb = STurbD_regr.predict(input_poly)[0]
