...
matplotlib.rcParams.update(
    {'font.family': 'STIXGeneral', 'mathtext.fontset': 'cm'})
# figsize=(10, 7.5)
# figsize=(3.5, 2.5)
# figsize = (7.16,2.5)
...
def create_knn_regression_model(x, y, min_x, max_x, plot_graph=False, x_label='x', y_label='y', plot_title='KNN Regression', n_neighbors=1, file_prefix=''):
    ...
    if plot_graph:
        ...
        plt.savefig(file_path + file_prefix +
                    '_knn_regression.pdf', bbox_inches='tight')
        plt.savefig(file_path + file_prefix +
                    '_knn_regression.png', bbox_inches='tight')
        plt.savefig(file_path + file_prefix +
                    '_knn_regression.svg', bbox_inches='tight')
...
def create_knn_regression_models(water_column_f, neighbors_list=[5, 5, 5, 5], plot_graph_list=[False, False, False, False], file_prefix=''):
    water_column_df = pd.read_csv(
        water_column_f, sep=',', low_memory=False).dropna()
    ...
    water_column_df_filtered = water_column_df.loc[:
                                                   max_depth_idx].loc[water_column_df['Depth m'] > 0]
    ...
    SDoD_knn = create_knn_regression_model(
        x_scaled, y, 0, 1, plot_graph_list[0], 'Normalized Depth', 'ODO mg/L', file_name, neighbors_list[0], file_prefix + 'do')

    y = water_column_df_filtered['Chlorophyll ug/L'].values
    SCD_knn = create_knn_regression_model(
        x_scaled, y, 0, 1, plot_graph_list[1], 'Normalized Depth', 'Chlorophyll ug/L', file_name, neighbors_list[1], file_prefix + 'chla')

    y = water_column_df_filtered['Temp °C'].values
    STempD_knn = create_knn_regression_model(
        x_scaled, y, 0, 1, plot_graph_list[2], 'Normalized Depth', 'Temp °C', file_name, neighbors_list[2], file_prefix + 'temp')

    y = water_column_df_filtered['Turbidity FNU'].values
    STurbD_knn = create_knn_regression_model(
        x_scaled, y, 0, 1, plot_graph_list[3], 'Normalized Depth', 'Turbidity FNU', file_name, neighbors_list[3], file_prefix + 'turb')
...
def create_linear_regression_model(x, y, min_x, max_x, plot_graph=False, x_label='x', y_label='y', plot_title='Linear Regression', file_prefix=''):
    ...
    if plot_graph:
        ...
        plt.savefig(file_path + file_prefix +
                    '_linear_regression.pdf', bbox_inches='tight')
        plt.savefig(file_path + file_prefix +
                    '_linear_regression.png', bbox_inches='tight')
        plt.savefig(file_path + file_prefix +
                    '_linear_regression.svg', bbox_inches='tight')
...
def create_polynomial_regression_model(x, y, min_x, max_x, degree=2, plot_graph=False, x_label='x', y_label='y', plot_title='Polynomial Linear Regression', file_prefix=''):
    ...
    if plot_graph:
        ...
        plt.savefig(file_path + file_prefix +
                    '_polynomial_regression.pdf', bbox_inches='tight')
        plt.savefig(file_path + file_prefix +
                    '_polynomial_regression.png', bbox_inches='tight')
        plt.savefig(file_path + file_prefix +
                    '_polynomial_regression.svg', bbox_inches='tight')
...
def create_polynomial_regression_models(water_column_f, degree_list=[2, 2, 2, 2], plot_graph_list=[False, False, False, False], file_prefix=''):
    ...
    y = water_column_df_filtered['ODO mg/L'].values
    SDoD_poly, SDoD_regr = hlp.create_polynomial_regression_model(
        x_scaled, y, 0, 1, degree_list[0], plot_graph_list[0], 'Normalized Depth', 'ODO mg/L', file_name, file_prefix + 'do')

    # Create a polynomial features transformer for model
    y = water_column_df_filtered['Chlorophyll ug/L'].values
    SCD_poly, SCD_regr = hlp.create_polynomial_regression_model(
        x_scaled, y, 0, 1, degree_list[1], plot_graph_list[1], 'Normalized Depth', 'Chlorophyll ug/L', file_name, file_prefix + 'chla')

    y = water_column_df_filtered['Temp °C'].values
    STempD_poly, STempD_regr = hlp.create_polynomial_regression_model(
        x_scaled, y, 0, 1, degree_list[2], plot_graph_list[2], 'Normalized Depth', 'Temp °C', file_name, file_prefix + 'temp')

    y = water_column_df_filtered['Turbidity FNU'].values
    STurbD_poly, STurbD_regr = hlp.create_polynomial_regression_model(
        x_scaled, y, 0, 1, degree_list[3], plot_graph_list[3], 'Normalized Depth', 'Turbidity FNU', file_name, file_prefix + 'turb')
...
def plot_3d_scatter_with_colorbar(x, y, z, z_label, plot_title, cmap='viridis', file_prefix=''):
    ...
    cbar = fig.colorbar(sc, location='top', shrink=0.6,
                        pad=-0.15, aspect=20, orientation='horizontal')
    ...
    # , bbox_inches='tight')
    plt.savefig(file_path + file_prefix + '_3D_Scatter.pdf')
    # , bbox_inches='tight')
    plt.savefig(file_path + file_prefix + '_3D_Scatter.png')
...
def plot_2d_scatter_with_colorbar(x, y, z, z_label, plot_title, cmap='viridis', file_prefix='', marker_size=5):
    ...
    # Add a colorbar to the plot
    cbar = fig.colorbar(sc, location='right', shrink=0.9,
                        pad=0.01, aspect=20, orientation='vertical')
    ...
    plt.savefig(file_path + file_prefix +
                '_2D_Scatter.pdf', bbox_inches='tight')
    plt.savefig(file_path + file_prefix +
                '_2D_Scatter.png', bbox_inches='tight')
    plt.savefig(file_path + file_prefix +
                '_2D_Scatter.svg', bbox_inches='tight')
...
def green_colormap(num_colors, alpha=0.9):
    start_color = np.array(
        [[1 / 255, 127 / 255, 0 / 255, alpha]])  # Forest green
    end_color = np.array(
        [[211 / 255, 241 / 255, 210 / 255, alpha]])  # Almost white
    t = np.linspace(0, 1, num_colors)
    colors = np.zeros((num_colors + 1, 4))
    # Brown or cinnamon
    colors[0] = np.array([[198 / 255, 172 / 255, 167 / 255, alpha * 0.6]])
...
def create_colormap(num_colors,
    ...
    if (type(high_concentration_color_or_cmap) == tuple or type(high_concentration_color_or_cmap) == list):
        high_concentration_color = np.array(
            list(high_concentration_color_or_cmap))
        ...
        colors = t*(high_concentration_color -
                    low_concentration_color) + low_concentration_color

    else:
        colors = high_concentration_color_or_cmap(
            np.linspace(0, 256, num_colors, dtype=int))
...
def plot_sdue(data, vmin, vmax, z_limit, plot_title, color_bar_label, file_prefix=''):
    ...
    # create the 3D plot
    x, y, z = data.shape
    X, Y, Z = np.meshgrid(np.arange(x), np.arange(
        y), np.arange(z) * -1, indexing='ij')
    fig = plt.figure()
    ax = fig.add_subplot(projection='3d')
    ...
    for i in range(z_limit):
        ...
        surf = ax.plot_surface(X[:, :, i], Y[:, :, i], Z[:, :, i], cmap=cmap,
                               facecolors=facecolors, linewidth=0, antialiased=False)
        surf.set_clim(vmin=vmin, vmax=vmax)
        surfs.append(surf)
    # add a colorbar
    cbar = fig.colorbar(surfs[0], location='top', shrink=0.6,
                        pad=-0.15, aspect=20, orientation='horizontal', extend='both')
    ...
    plt.title(plot_title)
    # , bbox_inches='tight')
    plt.savefig(file_path + file_prefix + '_surface_SDUE.pdf')
    # , bbox_inches='tight')
    plt.savefig(file_path + file_prefix + '_surface_SDUE.png')
    # , bbox_inches='tight')
    plt.savefig(file_path + file_prefix + '_surface_SDUE.svg')
    plt.show()


def plot_sdue_contourf(data, plot_title, color_bar_label,
                       file_prefix='', high_concentration_color_or_cmap=(0, 255/255, 0, 1.0),
                       low_concentration_color=(
    ...
    x, y = np.linspace(0, x_length, x_length), np.linspace(
        0, y_length, y_length)
    ...
    for i in range(0, n_layers):
        ...
        plot3d = ax.contourf(XX, YY, current_layer,
                             levels=Levels, colors=colors, offset=-i)
    ...
    cbar = fig.colorbar(plot3d, location='top', shrink=0.6,
                        pad=-0.15, aspect=15, orientation='horizontal')
    cbar.ax.set_xlabel(color_bar_label)
    cbar.ax.tick_params(labelsize=7)
    ...
    plt.title(plot_title)
    plt.savefig(file_path + file_prefix +
                '_contourf_SDUE.pdf', bbox_inches='tight')
    plt.savefig(file_path + file_prefix +
                '_contourf_SDUE.png', bbox_inches='tight')
    plt.savefig(file_path + file_prefix +
                '_contourf_SDUE.svg', bbox_inches='tight')
    plt.show()
